    ...
    $("#btn-pending").on("click", function () {
        ...
        if (!kp || !ps) {
            ...
            if (authSess == dirop) {
                if (dirop_act == "2") {
                    $("#contentData").html(
                        "<div class='form-group mb-4'><label for='pending_sampai'>Pending Sampai Tanggal:</label><p id='pending_sampai'>" +
                            ps +
                            "</p></div><div class='form-group mb-4'><label for='keterangan'>Alasan:</label><p id='keterangan'>" +
                            kp +
                            "</p></div><div class='form-group mb-4'><a href='" +
                            window.location.origin +
                            "/batal/pending/order-ebook/" +
                            id +
                            "' id='batalkan-pending' class='btn btn-danger'>Batalkan Pending</a></div>"
                    );
                } else {
                    $("#contentData").html(
                        "<div class='form-group mb-4'><label for='pending_sampai'>Pending Sampai Tanggal:</label><p id='pending_sampai'>" +
...
$(function () {
    ...
    function ajaxPendingProduksiEbook(data) {
        ...
        $.ajax({
            type: "POST",
            url:
                window.location.origin + "/penerbitan/order-ebook/ajax/pending",
    ...
    $("#fadd_Keterangan").on("submit", function (e) {
        ...
        if ($(this).valid()) {
            ...
            swal({
                title:
                    "Yakin order e-book #" + kode + "-" + judul + " dipending?",
