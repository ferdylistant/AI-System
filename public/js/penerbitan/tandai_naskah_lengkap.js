...
$(document).ready(function () {
    ...
    function loadDOM() {
        ...
        $("div.datesearchbox").html(
            '<div class="input-group"><input type="text" class="form-control pull-right deletable" id="datesearch" name="datefilter" placeholder="Filter by date range.."> </div>'
        );
        $("input.deletable")
            .wrap('<span class="deleteicon w-100"></span>')
            .after(
        ...
        document.getElementsByClassName("datesearchbox")[0].style.textAlign =
            "right";
                                ...
                                $("#naskahBelumDinilai")
                                    ...
                                    .animate(
                                        ...
                                        },
                                        {
                                            ...
                                            step: function (now) {
                                                $(this).text(
                                                    Math.ceil(now) + " Naskah"
                                                );
                                            },
                                        }
                                    );
                                $("#animate-count-prodev")
                                    .prop("Counter", 0)
                                    .animate(
                                        {
        ...
        $(".clear_field_jb").on("click", function (e) {
            ...
            var val = $.fn.dataTable.util.escapeRegex(
                $(".select-filter-jb").val()
            );
            tableNaskah
                .column($(".select-filter-jb").data("column"))
                .search(val ? val : "", true, false)
                .draw();
            if (checkReg === true) {
        ...
        $(".clear_field").click(function () {
            ...
            tableNaskah
                .column($(".select-filter").data("column"))
                .search(val ? val : "", true, false)
                .draw();
        ...
        $(".clear_field_kep").click(function () {
            ...
            var val = $.fn.dataTable.util.escapeRegex(
                $(".select-filter-kep").val()
            );
            tableNaskah
                .column($(".select-filter-kep").data("column"))
                .search(val ? val : "", true, false)
                .draw();
    ...
    let tableNaskah = $("#tb_Naskah").DataTable({
        ...
        aaSorting: [],
        dom:
            "<'row'<'col-sm-3' <'jalbuk'>><'col-sm-3'<'kelengkapan'>><'col-sm-3'<'keputusan'>><'col-sm-3' <'datesearchbox'>>>" +
            "<'row'<'col-sm-3'B><'col-sm-6 text-center' <'totalData'>><'col-sm-3'f>>" +
            "<'row'<'col-sm-12'tr>>" +
        ...
        drawCallback: () => {
            var tooltipTriggerList = [].slice.call(
                document.querySelectorAll(".tooltip-naskah")
            );
        ...
        $.ajax({
            ...
            success: (res) => {
                ...
                var tableNaskah = $("#tb_Naskah").DataTable({
                    ...
                    aaSorting: [],
                    dom:
                        "<'row'<'col-sm-4' <'jalbuk'>><'col-sm-4'<'kelengkapan'>><'col-sm-4'<'keputusan'>>>" +
                        "<'row'<'col-sm-3'B><'col-sm-6 text-center' <'totalData'>><'col-sm-3'f>>" +
                        "<'row'<'col-sm-12'tr>>" +
                        "<'row'<'col-sm-5'i><'col-sm-7'p>>",
                    scrollX: true,
    ...
    $("#tb_Naskah").on("click", ".btn-tracker", function (e) {
        ...
        $.ajax({
            ...
            success: function (data) {
                // console.log(data);
                $("#titleModalTracker").html(
                    '<i class="fas fa-file-signature"></i>&nbsp;Tracking Progress Naskah "' +
                        judul +
                        '"'
                );
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $("#titleModalNaskah").html(
                    '<i class="fas fa-history"></i>&nbsp;History Progress Naskah "' +
                        judul +
                        '"'
                );
    ...
    $(document).on("click", ".action-modal", function (e) {
        ...
        $.ajax({
            ...
            success: function (data) {
                $("#modalPenilaian")
                    .find("#titleModalPenilaian")
                    .text(data.titleModal);
                $("#modalPenilaian")
                    .find("#totalNaskahDinilai")
                    .text(data.totalNaskah);
                $("#modalPenilaian").find("#contentModal").html(data.content);
                $("#modalPenilaian")
                    .find("#boxContent")
                    .attr("style", "box-shadow:" + data.style);
                $("#modalPenilaian")
                    .find("#badgeTotal")
                    .addClass("badge " + data.class);
                $("#modalPenilaian")
                    .find("#scrollBar")
                    .addClass(
                        "container-fluid p-3 example-2 " + data.scroll + " thin"
                    );
