...
<div class="profile-widget-description">
    <div class="row">
        <div class="form-group col-12 mb-4">
            ...
            <input type="text" class="form-control" name="uedit_nama" value="{{$user->nama}}" placeholder="Nama Lengkap" required>
            <div id="err_uedit_nama"></div>
        </div>
        <div class="form-group col-12 col-md-6 mb-4">
            <label>Tanggal Lahir: </label>
            <input type="text" class="form-control datepicker" name="uedit_tanggal_lahir" value="{{$user->tanggal_lahir}}" placeholder="Hari Bulan Tahun">
            <div id="err_uedit_tanggal_lahir"></div>
        </div>
        <div class="form-group col-12 col-md-6 mb-4">
            <label>Tempat Lahir: </label>
            <input type="text" class="form-control" name="uedit_tempat_lahir" value="{{$user->tempat_lahir}}" placeholder="Kota / Daerah Kelahiran">
        ...
        <div class="form-group col-12 col-md-6 mb-4">
            ...
            <input type="text" class="form-control" name="uedit_email" value="{{$user->email}}" placeholder="Email Aktif" required>
        ...
        <div class="form-group col-12 col-md-6 mb-4">
            ...
            <select class="form-control select2" name="uedit_cabang" required>
                ...
                @foreach($lcab as $c)
                <option value="{{$c->id}}" {{$c->id==$user->cabang_id?'Selected':''}}>{{$c->nama}}</option>
                @endforeach
            </select>
        ...
        <div class="form-group col-12 col-md-6 mb-4">
            ...
            <select class="form-control select2" name="uedit_divisi" required>
                <option label="Pilih"></option>
                @foreach($ldiv as $d)
                <option value="{{$d->id}}" {{$d->id==$user->divisi_id?'Selected':''}}>{{$d->nama}}</option>
                @endforeach
            </select>
        ...
        <div class="form-group col-12 col-md-6 mb-4">
            ...
            <select class="form-control select2" name="uedit_jabatan" required>
                <option label="Pilih"></option>
                @foreach($ljab as $j)
                <option value="{{$j->id}}" {{$j->id==$user->jabatan_id?'Selected':''}}>{{$j->nama}}</option>
                @endforeach
            </select>
