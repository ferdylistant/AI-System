    ...
    @foreach ($pn_pemasaran as $p)
        <div class="row">
            <div class="col-12 mb-3">
                <h5 class="section-title">Penilaian oleh: <a class="text-primary text-decoration-none"
                        href="{{ url('manajemen-web/user/' . $p->created_by) }}"><b>{{ \Str::ucfirst($p->nama_manager) }}</b></a>
                </h5>
                @if ($history)
                    <a class="btn btn-sm btn-dark float-right btn-history-penilaian" href="javascript:void(0)"
                        data-toggle="modal" data-target="#modalHistoryPenilaian" data-pic="MPM"
                        data-naskahid="{!! $naskah->id !!}"><i class="fas fa-folder-open"></i> Riwayat Penilaian</a>
                @endif
            </div>
            ...
            <div class="form-group col-12">
                ...
                <input type="text" class="form-control" value="{{ $p->potensi_dana }}" placeholder="Potensi Dana"
                    readonly>
            </div>
            <div class="form-group col-12">
                ...
                @foreach ($arrDSTB as $v)
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="checkbox" id="cb_{{ $v }}"
                            value="{{ $v }}" {{ in_array($v, json_decode($p->ds_tb)) ? 'checked' : '' }}
                            disabled>
                ...
                @foreach ($pilar as $pl)
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="checkbox"
                            {{ in_array($pl->options, json_decode($p->pilar)) ? 'checked' : '' }} disabled>
    ...
    <form id="fpn3">
        <div class="row">
            ...
            <div class="form-group col-12 mb-4">
                ...
                @foreach ($pilar as $p)
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="checkbox" id="P_{{ $p->options }}" name="pn3_pilar[]"
                            value="{{ $p->options }}">
        ...
        <div class="row">
            @if ($history)
                <div class="col-12 mb-3">
                    <a class="btn btn-sm btn-dark float-right btn-history-penilaian" href="javascript:void(0)"
                        data-toggle="modal" data-target="#modalHistoryPenilaian" data-pic="MPM"
                        data-naskahid="{!! $naskah->id !!}"><i class="fas fa-folder-open"></i> Riwayat
                        Penilaian</a>
                </div>
            @endif
            <input type="hidden" name="pn3_naskah_id" value="{{ $naskah->id }}">
            ...
            <div class="form-group col-12 mb-4">
                <label>Potensi Dana: <span class="text-danger">*</span></label>
                <input type="text" class="form-control" name="pn3_potensi_dana"
                    value="{{ $pn_pemasaran->potensi_dana }}" placeholder="Potensi Dana" disabled>
                <div id="err_pn3_potensi_dana"></div>
            </div>
            <div class="form-group col-12 mb-4">
                <label class="d-block">DS / TB: <span class="text-danger">*</span></label>
                @foreach ($arrDSTB as $v)
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="checkbox" id="cb_{{ $v }}"
                            name="pn3_dstb[]" value="{{ $v }}"
                            {{ in_array($v, json_decode($pn_pemasaran->ds_tb)) ? 'checked' : '' }} disabled>
                        <label class="form-check-label" for="cb_{{ $v }}">{{ $v }}</label>
                    </div>
                @endforeach
                <div id="err_pn3_dstb"></div>
            ...
            <div class="form-group col-12 mb-4">
                ...
                @foreach ($pilar as $key => $p)
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="checkbox" id="P_{{ $p->options }}"
                            name="pn3_pilar[]" value="{{ $p->options }}"
                            {{ in_array($p->options, json_decode($pn_pemasaran->pilar)) ? 'checked' : '' }} disabled>
                        <label class="form-check-label" for="P_{{ $p->options }}">{{ $p->options }}</label>
                    </div>
                @endforeach
                <div id="err_pn3_pilar"></div>
