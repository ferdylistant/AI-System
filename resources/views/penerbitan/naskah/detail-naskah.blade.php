    ...
    <script>
        $(function() {
            ...
            $('#pn_Prodev').on('click', '.btn_addSaranJudul', function(e) {
                ...
                $(e.currentTarget).removeClass('btn-success btn_addSaranJudul').addClass(
                    'btn-danger btn_rmvSaranJudul').text('Hapus?')
                el.after(
                    '<tr><td><input type="text" class="form-control" name="pn1_usulan_judul[]" required></td>' +
            ...
            function ajaxSubmitDateTimeline(data) {
                ...
                $.ajax({
                    ...
                    success: function(result) {
                        console.log(result)
                        notifToast('success', 'Timeline berhasil disimpan!', method_ == 'add' ? true :
                            false);
                    },
                    error: function(err) {
