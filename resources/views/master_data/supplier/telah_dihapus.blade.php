...
@section('content')
    <section class="section">
        <div class="section-header">
            ...
            <h1>Data Supplier Yang Telah Dihapus</h1>
        </div>
                    ...
                    <div class="card card-danger">
                        <div class="card-body">
                            <div class="col-12 table-responsive">
                                <table class="table table-striped" id="tb_Supplier" style="width:100%">
                                </table>
                            </div>
                        </div>
                    </div>
...
@section('jsNeeded')
    <script>
        $(function() {
            let tableDelSupplier = $('#tb_Supplier').DataTable({
                responsive: true,
                processing: true,
                serverSide: true,
                language: {
                ...
                ajax: {
                    url: "{{ url('master/supplier/supplier-telah-dihapus') }}",
                },
                columns: [{
                        data: 'no',
                        name: 'no',
            ...
            // Restore Supplier Start
            $(document).ready(function() {
                function ajaxRestoreSupplier(data) {
                    $.ajax({
                        type: "POST",
                        url: window.location.origin +
                            "/master/supplier/restore",
                        data: data,
                        beforeSend: function() {
                            $(".btn_ResSupplier")
                                .prop("disabled", true)
                ...
                $(document).on("click", ".btn_ResSupplier", function(e) {
                    let nama = $(this).data("nama"),
                        id = $(this).data("id");
                    swal({
                        text: "Kembalikan data supplier (" + nama + ")?",
                        icon: "warning",
                        buttons: true,
                        dangerMode: true,
                    }).then((confirm_) => {
