...
@section('content')
    <section class="section">
        <div class="section-header">
            <h1>Data Operator Mesin</h1>
            @if (Gate::allows('do_create', 'buat-operator-mesin'))
                <div class="section-header-button">
                    <div class="card-header-action">
                        <button id="btn_AddOMesin" class="btn btn-success" data-toggle="modal" data-target="#md_AddOMesin"
                            data-backdrop="static">Tambah</button>
                        <a href="{{ route('om.telah_dihapus') }}" class="btn btn-danger">Format Buku Telah Dihapus</a>
                    </div>
                </div>
            @endif
        </div>
    ...
    <!-- Modal Format Buku -->
    @include('master_data.operator_mesin.include.modal_add_operator_mesin')
    @include('master_data.operator_mesin.include.modal_edit_operator_mesin')
    @include('master_data.operator_mesin.include.modal_history_operator_mesin')
@endsection
...
@section('jsNeeded')
    <script src="{{ url('js/master_data/operator_mesin.js') }}"></script>
@endsection
