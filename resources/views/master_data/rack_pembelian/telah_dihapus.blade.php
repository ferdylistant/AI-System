...
@section('content')
    <section class="section">
        <div class="section-header">
            ...
            <h1>Data Rack Pembelian Yang Telah Dihapus</h1>
        </div>
...
@section('jsNeeded')
    <script>
        $(function() {
            let tableDelRack = $('#tb_Rack').DataTable({
                responsive: true,
                processing: true,
                serverSide: true,
                language: {
                ...
                ajax: {
                    url: "{{ url('master/rack-pembelian/rack-pembelian-telah-dihapus') }}",
                },
                columns: [{
                        data: 'no',
                        name: 'no',
            ...
            $(document).ready(function() {
                function ajaxRestoreRack(data) {
                    $.ajax({
                        type: "POST",
                        url: window.location.origin +
                            "/master/rack-pembelian/restore",
                        data: data,
                        beforeSend: function() {
                            $(".btn_ResRack")
                                .prop("disabled", true)
                ...
                $(document).on("click", ".btn_ResRack", function(e) {
                    let nama = $(this).data("nama"),
                        id = $(this).data("id");
                    swal({
                        text: "Kembalikan data rack (" + nama + ")?",
                        icon: "warning",
                        buttons: true,
                        dangerMode: true,
                    }).then((confirm_) => {
