...
<section class="section">
    <div class="section-header">
        <h1>Data Kelompok Buku</h1>
        @if(Gate::allows('do_create', 'buat-kelompok-buku'))
        <div class="section-header-button">
            <a href="{{ route('kb.create')}}" class="btn btn-success">Tambah</a>
        </div>
        @endif
    </div>
                ...
                <div class="card card-primary">
                    <div class="card-body">
                        <div class="col-12 table-responsive">
                            <table class="table table-striped" id="tb_Kbuku" style="width:100%">
                            </table>
                        </div>
                    </div>
                </div>
...
<div id="md_KelompokBukuHistory" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="titleModal" aria-hidden="true">
...
@section('jsNeeded')
<script src="{{url('js/delete_kbuku.js')}}"></script>
<script>
    $(function() {
         $('#tb_Kbuku').DataTable({
            responsive: true,
            ...
            ajax: {
                url: "{{ url('master/kelompok-buku') }}",
                data: {"request_": "table-kbuku"}
            },
            columns: [
                { data: 'kode', name: 'kode', title: 'Kode' },
                { data: 'nama', name: 'nama', title: 'Nama Kelompok' },
...
<script>
    $(function(){
        $('#tb_Kbuku').on('click','.btn-history',function(e){
            var data = $(this).data('id');
            $.post("{{route('kb.history')}}", {id: data}, function(data){
                $('#dataHistory').empty();
                $.each(data, function(k, v) {
                $('#dataHistory').append(`<span class="ticket-item">
                        <div class="ticket-title">
                            <h6><span class="bullet"></span> Kelompok buku '`+v.kelompok_buku_history+`' diubah menjadi '`+v.kelompok_buku_new+`'.</h6>
                        </div>
