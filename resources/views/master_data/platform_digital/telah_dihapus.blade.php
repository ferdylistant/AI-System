...
@section('jsNeeded')
    ...
    <script>
        $(function() {
            $('#tb_Platform').on('click', '#hapus-platform', function(e) {
                var data = $(this).data('id');
                $.post("{{ route('platform.restore') }}", {
                    id: data
                }, function(data) {
                    $('#dataHistory').empty();
                    $.each(data, function(k, v) {
                        ...
                        <div class="ticket-title">
                            <h6><span class="bullet"></span> Platform '` + v.platform_history + `' diubah menjadi '` +
                            v.platform_new +
                            `'.</h6>
                        </div>
                        <div class="ticket-info">
                            <div class="text-muted">Modified by <a href="{{ url('/manajemen-web/user/`+v.author_id+`') }}">` +
                            v.nama + `</a></div>
