...
@section('content')
    <section class="section">
        <div class="section-header">
            ...
            <h1>Data Category Gudang Material Yang Telah Dihapus</h1>
        </div>
...
@section('jsNeeded')
    <script>
        $(function() {
            let tableDelCategory = $('#tb_Category').DataTable({
                responsive: true,
                processing: true,
                serverSide: true,
                language: {
                ...
                ajax: {
                    url: "{{ url('master/category-gudang-material/category-telah-dihapus') }}",
                },
                columns: [{
                        data: 'no',
                        name: 'no',
            ...
            $(document).ready(function() {
                function ajaxRestoreCategory(data) {
                    $.ajax({
                        type: "POST",
                        url: window.location.origin +
                            "/master/category-gudang-material/restore",
                        data: data,
                        beforeSend: function() {
                            $(".btn_ResCategory")
                                .prop("disabled", true)
                ...
                $(document).on("click", ".btn_ResCategory", function(e) {
                    let nama = $(this).data("nama"),
                        id = $(this).data("id");
                    swal({
                        text: "Kembalikan data category (" + nama + ")?",
                        icon: "warning",
                        buttons: true,
                        dangerMode: true,
                    }).then((confirm_) => {
