...
@section('content')
    <section class="section">
        <div class="section-header">
            ...
            <h1>Data Operator Gudang Yang Telah Dihapus</h1>
        </div>
...
@section('jsNeeded')
    <script>
        $(function() {
            let tableDelOperatorGudang = $('#tb_OperatorGudang').DataTable({
                responsive: true,
                processing: true,
                serverSide: true,
                language: {
                ...
                ajax: {
                    url: "{{ url('master/operator-gudang/operator-gudang-telah-dihapus') }}",
                },
                columns: [{
                        data: 'no',
                        name: 'no',
            ...
            $(document).ready(function() {
                function ajaxRestoreOperatorGudang(data) {
                    $.ajax({
                        type: "POST",
                        url: window.location.origin +
                            "/master/operator-gudang/restore",
                        data: data,
                        beforeSend: function() {
                            $(".btn_ResOperatorGudang")
                                .prop("disabled", true)
                        ...
                        success: function(result) {
                            ...
                            if (result.status == "success") {
                                tableDelOperatorGudang.ajax.reload();
                            }
                        },
                        error: function(err) {
                ...
                $(document).on("click", ".btn_ResOperatorGudang", function(e) {
                    let nama = $(this).data("nama"),
                        id = $(this).data("id");
                    swal({
                        text: "Kembalikan data rak (" + nama + ")?",
                        icon: "warning",
                        buttons: true,
                        dangerMode: true,
                    }).then((confirm_) => {
                        if (confirm_) {
                            ajaxRestoreOperatorGudang({
                                id: id
                            })
                        }
                    });
