...
@section('content')
    <section class="section">
        <div class="section-header">
            ...
            <h1>Data Satuan Yang Telah Dihapus</h1>
        </div>
...
@section('jsNeeded')
    <script>
        $(function() {
            let tableDelSatuan = $('#tb_Satuan').DataTable({
                responsive: true,
                processing: true,
                serverSide: true,
                language: {
                ...
                ajax: {
                    url: "{{ url('master/satuan/satuan-telah-dihapus') }}",
                },
                columns: [{
                        data: 'no',
                        name: 'no',
            ...
            // Restore Satuan Start
            $(document).ready(function() {
                function ajaxRestoreSatuan(data) {
                    $.ajax({
                        type: "POST",
                        url: window.location.origin +
                            "/master/satuan/restore",
                        data: data,
                        beforeSend: function() {
                            $(".btn_ResSatuan")
                                .prop("disabled", true)
                ...
                $(document).on("click", ".btn_ResSatuan", function(e) {
                    let nama = $(this).data("nama"),
                        id = $(this).data("id");
                    swal({
                        text: "Kembalikan data satuan (" + nama + ")?",
                        icon: "warning",
                        buttons: true,
                        dangerMode: true,
                    }).then((confirm_) => {
