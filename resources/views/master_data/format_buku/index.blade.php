...
<section class="section">
    <div class="section-header">
        ...
        <div class="section-header-button">
            <a href="{{ route('fb.create')}}" class="btn btn-success">Tambah</a>
        </div>
        @endif
    </div>
...
<div id="md_FormatBukuHistory" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="titleModal" aria-hidden="true">
...
@section('jsNeeded')
<script src="{{url('js/delete_fbuku.js')}}"></script>
<script>
    $(function() {
         $('#tb_Fbuku').DataTable({
            responsive: true,
            ...
            ajax: {
                url: "{{ url('master/format-buku') }}",
                data: {"request_": "table-format-buku"}
            },
            columns: [
                { data: 'no', name: 'no', title: 'No' },
                { data: 'jenis_format', name: 'jenis_format', title: 'Jenis Format' },
...
<script>
    $(function(){
        $('#tb_Fbuku').on('click','.btn-history',function(e){
            ...
            $.post("{{route('fb.history')}}", {id: data}, function(data){
                $('#dataHistory').empty();
                $.each(data, function(k, v) {
                $('#dataHistory').append(`<span class="ticket-item">
                        <div class="ticket-title">
                            <h6><span class="bullet"></span> Format buku '`+v.jenis_format_history+`' diubah menjadi '`+v.jenis_format_new+`'.</h6>
                        </div>
