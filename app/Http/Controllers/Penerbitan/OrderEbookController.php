...
class OrderEbookController extends Controller
{
    public function index(Request $request)
    {
        if ($request->ajax()) {
            ...
            if ($request->has('count_data')) {
                ...
                $showStatus = DB::table('order_ebook as ob')
                    ->join('deskripsi_turun_cetak as dtc', 'dtc.id', '=', 'ob.deskripsi_turun_cetak_id')
                    ->join('pracetak_setter as ps', 'ps.id', '=', 'dtc.pracetak_setter_id')
                    ->join('pracetak_cover as pc', 'pc.id', '=', 'dtc.pracetak_cover_id')
                    ->join('deskripsi_final as df', 'df.id', '=', 'ps.deskripsi_final_id')
                    ->join('deskripsi_cover as dc', 'dc.id', '=', 'pc.deskripsi_cover_id')
                    ->join('deskripsi_produk as dp', 'dp.id', '=', 'dc.deskripsi_produk_id')
                    ->where('ob.id', $request->id)
                    ->select('ob.*', 'dp.judul_final')
                    ->first();
    ...
    public function ajaxRequest(Request $request, $cat)
    {
        ...
        switch ($cat) {
            case 'approve':
                return $this->approvalOrderEbook($request,$data);
                break;
            case 'decline':
                return $this->declineOrderEbook($request,$data);
                break;
            case 'approve-detail':
            ...
            case 'lihat-history-order-ebook':
                ...
                break;
            case 'lihat-tracking':
                return $this->lihatTrackingEbook($request);
                break;
            default:
                abort(500);
        }
        ...
        try {
            ...
            if ($request->status == 'Selesai') {
                ...
                $permissionAdmin = DB::table('permissions as p')->join('user_permission as up','up.permission_id','=','p.id')
                        ...
                        ->where('p.id','=','c21495eca0d44776aeacf431dc9fb0e1')
                ...
                $permissionAdmin = (object)collect($permissionAdmin)->map(function($item) use ($data) {
                    ...
                    ->where('title','Lengkapi data order ebook untuk naskah "'.$data->judul_final.'".');
                    if (is_null($cekTodo->first())) {
                        return DB::table('todo_list')->insert([
                            'form_id' => $data->id,
                            'users_id' => $item->user_id,
                            'title' => 'Lengkapi data order ebook untuk naskah "'.$data->judul_final.'".',
                            'link' => '/penerbitan/order-ebook/edit?order='.$data->id.'&naskah='.$data->kode,
                            'status' => '1'
                        ]);
                    } else {
                        $cekTodo->update([
                ...
                //INSERT TODO LIST PRODUKSI UPLOAD E-BOOK
                    //? BELUM
                ...
                if ($request->status == 'Proses') {
                    ...
                    $dataPerm = [
                        [ //Penerbitan
                            'id_perm'  => '171e6210418440a8bf4d689841d0f32c',
                            'title' => 'Persetujuan order ebook sebagai perwakilan Departemen Penerbitan untuk naskah "'.$data->judul_final.'".'
                        ],
                        [ //Pemasaran
                            'id_perm'  => '4cea10b3a4434bc3b342407a78a9ab2a',
                            'title' => 'Persetujuan order ebook sebagai perwakilan Departemen Operasional dan Pemasaran untuk naskah "'.$data->judul_final.'".'
                        ],
                        [ //Keuangan
                            'id_perm'  => '78712deb909d4d88af7f098c0fcf6857',
                            'title' => 'Persetujuan order ebook sebagai perwakilan Departemen Keuangan untuk naskah "'.$data->judul_final.'".'
                        ],
                        [ //Direktur Utama
                            'id_perm'  => '8f53727c763849aab80c1513505decf8',
                            'title' => 'Persetujuan order ebook sebagai perwakilan Direktur Utama untuk naskah "'.$data->judul_final.'".'
                        ],
                    ];
                        ...
                        (object)collect($depart)->map(function($item) use ($data) {
                            ...
                            if (!is_null($cekEksisTodo->first())){
                                ...
                                DB::table('todo_list')->insert([
                                    ...
                                    'link' => '/penerbitan/order-ebook/detail?order=' . $data['id'] . '&naskah=' . $data['kode'],
                                    'status' => $status
                                ]);
                            }
                        })->all();
    ...
    protected function approvalOrderEbook($request,$data)
    {
        try {
            if (is_null($data)) {
                return response()->json([
            ...
            switch ($request->type_departemen) {
                case 'Penerbitan':
                    ...
                    $raw = ['Marketing & Ops'];
                    //?Dep Penerbitan
                    $permissionId = '171e6210418440a8bf4d689841d0f32c';
                    $titleTodo = 'Persetujuan order ebook sebagai perwakilan Departemen Penerbitan untuk naskah "'.$data->judul_final.'".';
                ...
                case 'Marketing & Ops':
                    ...
                    $raw = ['Keuangan'];
                    //?Dep Pemasaran
                    $permissionId = '4cea10b3a4434bc3b342407a78a9ab2a';
                    $titleTodo = 'Persetujuan order ebook sebagai perwakilan Departemen Operasional dan Pemasaran untuk naskah "'.$data->judul_final.'".';
                    break;
                case 'Keuangan':
                    $id_notif = Str::uuid()->getHex();
                    $raw = ['Direktur Utama'];
                    //?Dep Keuangan
                    $permissionId = '78712deb909d4d88af7f098c0fcf6857';
                    $titleTodo = 'Persetujuan order ebook sebagai perwakilan Departemen Keuangan untuk naskah "'.$data->judul_final.'".';
                    break;
                case 'Direktur Utama':
                    $id_notif = Str::uuid()->getHex();
                    $raw = ['Penerbitan', 'Marketing & Ops', 'Direktur Keuangan'];
                    //?Update Direktur Utama
                    $permissionId = '8f53727c763849aab80c1513505decf8';
                    $titleTodo = 'Persetujuan order ebook sebagai perwakilan Direktur Utama untuk naskah "'.$data->judul_final.'".';
                    break;
            }
            $dataUse = [
                'id' => $data->id,
            ...
            (object)collect($depart)->map(function($item) use ($dataUse) {
                ...
                if (!is_null($cekEksis->first())){
                    ...
                    DB::table('todo_list')->insert([
                        ...
                        'users_id' => $item->user_id,
                        'title' => $dataUse['title'],
                        'link' => '/penerbitan/order-ebook/detail?order=' . $dataUse['id'] . '&naskah=' . $dataUse['kode'],
                        'status' => '1'
                    ]);
                }
            })->all();
    ...
    protected function declineOrderEbook($request,$data)
    {
        try {
            if (is_null($data)) {
                return response()->json([
    ...
    protected function lihatTrackingEbook($request)
    {
        if ($request->ajax()) {
            $html = '';
            $id = $request->id;
