...
class OperatorMesinController extends Controller
{
    public function index(Request $request)
    {
        if ($request->ajax()) {
            $data = DB::table('proses_produksi_master')
                ...
                ->orderBy('nama', 'asc')
                ->get();
            $update = Gate::allows('do_update', 'ubah-operator-mesin');
            $start = 1;
                ...
                ->addColumn('action', function ($data) use ($update) {
                    if ($update) {
                        $btn = '<a href="javascript:void(0)" class="d-block btn btn-sm btn-warning btn-icon mr-1 mt-1"
                                    data-toggle="modal" data-target="#md_EditOperatorMesin" data-backdrop="static"
                                    data-id="' . $data->id . '" data-nama="' . $data->nama . '"
                                    data-toggle="tooltip" title="Edit Data">
                                    <div><i class="fas fa-edit"></i></div></a>';
                    }
                    if (Gate::allows('do_delete', 'hapus-operator-mesin')) {
                        $btn .= '<a href="javascript:void(0)" class="d-block btn btn-sm btn_DelOperatorMesin btn-danger btn-icon mr-1 mt-1"
                        data-toggle="tooltip" title="Hapus Data"
                        data-id="' . $data->id . '" data-nama="' . $data->nama . '">
                        <div><i class="fas fa-trash-alt"></i></div></a>';
                    }
                    if (Auth::user()->cannot('do_update', 'ubah-operator-mesin') && Auth::user()->cannot('do_delete', 'hapus-operator-mesin')) {
        ...
        return view('master_data.operator_mesin.index', [
            'title' => 'Operator Mesin Produksi'
        ]);
    }
    public function oMesinTelahDihapus(Request $request)
    {
        if ($request->ajax()) {
            $data = DB::table('proses_produksi_master')
                ->where('type', 'O')
                ...
                ->orderBy('nama', 'asc')
                ->get();
            $update = Gate::allows('do_delete', 'hapus-operator-mesin');
            ...
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($data) use ($update) {
                    if ($update) {
                        ...
                        class="d-block btn btn-sm btn_ResOperatorMesin btn-dark btn-icon""
        ...
        return view('master_data.operator_mesin.telah_dihapus', [
            'title' => 'Operator Mesin Telah Dihapus',
        ]);
    }
    public function createOmesin(Request $request)
    {
        if ($request->ajax()) {
            if ($request->isMethod('POST')) {
                try {
                    ...
                    return response()->json([
                        'status' => 'success',
                        'message' => 'Data operator mesin berhasil ditambahkan!',
                    ]);
                } catch (\Exception $e) {
                    DB::rollBack();
                    return response()->json([
    ...
    public function updateOmesin(Request $request)
    {
        if ($request->ajax()) {
            if ($request->isMethod('POST')) {
                try {
                    ...
                    return response()->json([
                        'status' => 'success',
                        'message' => 'Data operator mesin berhasil diubah!',
                    ]);
                } catch (\Exception $e) {
                    DB::rollBack();
                    return response()->json([
    ...
    public function deleteOmesin(Request $request)
    {
        try {
            $id = $request->id;
            //Check Relasi
            $track = DB::table('proses_produksi_track')->get();
            foreach ($track as $t) {
                if (in_array($id, json_decode($t->operator))) {
                    return response()->json([
                        'status' => 'error',
                        'message' => 'Operator mesin tidak bisa dihapus karena telah terpakai di naskah yang sedang diproses!'
                    ]);
                }
            }
            ...
            return response()->json([
                'status' => 'success',
                'message' => 'Berhasil hapus data jenis format!'
            ]);
        } catch (\Exception $e) {
            DB::rollBack();
            return response()->json([
    ...
    public function lihatHistoryOmesin(Request $request)
    {
        if ($request->ajax()) {
            $html = '';
            $id = $request->id;
            ...
            foreach ($data as $d) {
                switch ($d->type_history) {
                    case 'Update':
                        $html .= '<span class="ticket-item" id="newAppend">
                            <div class="ticket-title">
                                <span><span class="bullet"></span> Operator mesin <b class="text-dark">' . $d->nama_his . '</b> diubah menjadi <b class="text-dark">' . $d->nama_new . '</b>.</span>
                            </div>
                    ...
                    case 'Delete':
                        $html .= '<span class="ticket-item" id="newAppend">
                            <div class="ticket-title">
                                <span><span class="bullet"></span> Data operator mesin dihapus pada <b class="text-dark">' . Carbon::parse($d->modified_at)->translatedFormat('l, d M Y, H:i') . '</b>.</span>
                            </div>
                            <div class="ticket-info">
                                <div class="text-muted pt-2">Modified by <a href="' . url('/manajemen-web/user/' . $d->author_id) . '">' . $d->nama . '</a></div>
                                <div class="bullet pt-2"></div>
                    ...
                    case 'Restore':
                        $html .= '<span class="ticket-item" id="newAppend">
                                <div class="ticket-title">
                                    <span><span class="bullet"></span> Data operator mesin direstore pada <b class="text-dark">' . Carbon::parse($d->modified_at)->translatedFormat('l, d M Y, H:i') . '</b>.</span>
                                </div>
                                <div class="ticket-info">
                                    <div class="text-muted pt-2">Modified by <a href="' . url('/manajemen-web/user/' . $d->author_id) . '">' . $d->nama . '</a></div>
                                    <div class="bullet pt-2"></div>
    ...
    public function restoreOmesin(Request $request)
    {
        try {
            $id = $request->id;
            $insert = [
            ...
            return response()->json([
                'status' => 'success',
                'message' => 'Berhasil mengembalikan operator mesin!'
            ]);
        } catch (\Exception $e) {
            DB::rollBack();
            return response()->json([
