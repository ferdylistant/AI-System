...
class GolonganController extends Controller
{
            ...
            return DataTables::of($data)
                ...
                ->addColumn('history', function ($data) {
                    ...
                    if ($historyData->isEmpty()) {
                        ...
                        $date = '<button type="button" class="btn btn-sm btn-dark btn-icon mr-1" data-type="history"
                        data-toggle="modal" data-target="#md_Golongan" data-backdrop="static" data-id="' . $data->id . '" data-nama="' . $data->nama . '"><i class="fas fa-history"></i>&nbsp;History</button>';
                        return $date;
                    }
                })
                ->addColumn('action', function ($data) use ($update) {
                    if ($update) {
                        $btn = '<a href="#" class="d-block btn btn-sm btn-warning btn-icon mr-1 mt-1" data-type="edit"
                    ...
                    if (Gate::allows('do_delete', 'hapus-golongan')) {
                        $btn .= '<a href="#" class="d-block btn btn-sm btn_DelGolongan btn-danger btn-icon mr-1 mt-1"
        ...
        if ($request->ajax()) {
            switch ($request->cat) {
                case 'add':
                    ...
                    break;
                case 'edit':
                    return $this->showUpdateGolongan($request);
                    break;
                case 'history':
    ...
    public function createGolongan(Request $request)
    {
        if ($request->ajax()) {
            if ($request->isMethod('POST')) {
                $last = DB::table('golongan')->whereNull('deleted_at')->orderBy('created_at', 'desc')->first();
                if (is_null($last)) {
                    $kode = 'KB001';
                } else {
                    // $last = DB::table('golongan')->whereNull('deleted_at')->max('kode')->first();
                ...
                $insert = [
                    'params' => 'Insert History Create Golongan',
                    'golongan_id' => $id,
                    'type_history' => 'Create',
                    'kode' => $kode,
                    'nama_golongan' => $request->nama_golongan,
        ...
        return view('master_data.golongan.create', [
    ...
    public function updateGolongan(Request $request)
    {
        if ($request->ajax()) {
            if ($request->isMethod('POST')) {
                $history = DB::table('golongan')->where('id', $request->edit_id)->first();
                $update = [
                ...
                $insert = [
                    ...
                    'type_history' => 'Update',
                    'golongan_id' => $request->edit_id,
                    'golongan_history' => $history->nama,
                    'golongan_new' => $request->edit_nama,
                    'author_id' => auth()->user()->id,
                    'modified_at' => Carbon::now('Asia/Jakarta')->toDateTimeString()
                ];
                event(new MasterDataEvent($insert));
    ...
    public function deleteGolongan(Request $request)
    {
        try {
            $id = $request->id;
            //Check Relasi
            $relation = DB::table('penerbitan_naskah')->where('golongan_id', $id)->first();
            if (!is_null($relation)) {
                return response()->json([
                    'status' => 'error',
    ...
    public function lihatHistoryGolongan(Request $request)
    {
        if ($request->ajax()) {
            $html = '';
            $id = $request->id;
            $data = DB::table('golongan_history as kbh')
                ->join('golongan as kb', 'kb.id', '=', 'kbh.golongan_id')
                ->join('users as u', 'u.id', '=', 'kbh.author_id')
                ->where('kbh.golongan_id', $id)
                ->select('kbh.*', 'u.nama')
                ->orderBy('kbh.id', 'desc')
                ->paginate(2);
                ...
                switch ($d->type_history) {
                    case 'Create':
                        ...
                        <div class="ticket-title">
                            <span><span class="bullet"></span> Kelompok Buku <b class="text-dark">' . $d->golongan_name  . '</b> dengan kode <b class="text-dark">' . $d->kode . '</b> ditambahkan .</span>
                        </div>
                    ...
                    case 'Update':
                        $html .= '<span class="ticket-item" id="newAppend">
                            <div class="ticket-title">
                                <span><span class="bullet"></span> Kelompok Buku <b class="text-dark">' . $d->golongan_history . '</b> diubah menjadi <b class="text-dark">' . $d->golongan_new . '</b>.</span>
                            </div>
                            <div class="ticket-info">
                                <div class="text-muted pt-2">Modified by <a href="' . url('/manajemen-web/user/' . $d->author_id) . '">' . $d->nama . '</a></div>
                                <div class="bullet pt-2"></div>
    ...
    private function showCreateGolongan()
    {
        $html = '';
        ...
        <button type="submit" class="btn btn-sm btn-success" form="fm_addGolongan">Simpan</button>';
